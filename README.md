...
> **Текущий статус: release candidate `0.1.9`.** Версия `0.1.9` сохраняет проверенное optimizer-поведение `0.1.7`, добавляет переносимую обработку SQLite sidecar race на Linux/macOS и использует жёсткий allowlist для source distribution. Рекомендуется для ограниченного тестирования в отдельном Hermes profile с заранее подготовленным rollback. Это пока не универсальная production-рекомендация для всех профилей, инструментов и операционных систем.
...
```bash
uv pip install --python <isolated-python> dist/hermes_context_optimizer-0.1.9-py3-none-any.whl
```
